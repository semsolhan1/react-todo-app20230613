...
const TodoTemplate = () => {
  ...
  //자식 컴포넌트가 부모 컴포넌트에게 데이터를전달할 때는
  //props 사용이 불가능.
  //부모 컴포넌트에서 함수를 선언(매개변수 꼭 선언!) -> props로 함수를 전달
  //자식 컴포넌트에서 전달받은 함수를 호출하면서매개값으로 데이터를 전달
  ...
  const removeTodo = id => {
    ...
    // setTodos(todos.useEffect(todo => todo.id !== id)); (x)
